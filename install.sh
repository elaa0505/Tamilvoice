#!/bin/bash
DOWNLOAD_PATH=/home/elango/Desktop/Code/t2s/elaa6/packages
COMPILE_PATH=/home/elango/Desktop/Code/t2s/elaa6/compiled


#Register here http://htk.eng.cam.ac.uk/download.shtml and get a username and password

HTKUSER=Elango05
HTKPASSWORD=Qc2hihsv
...
cd $DOWNLOAD_PATH
wget https://www.iitm.ac.in/donlab/tts/downloads/voices/hts23/ssn_hts_demo_Tamil_male.tgz --no-check-certificate
...
cd $DOWNLOAD_PATH
wget https://nchc.dl.sourceforge.net/project/sp-tk/SPTK/SPTK-3.10/SPTK-3.10.tar.gz --no-check-certificate
...
mkdir hts-htk
cd hts-htk
wget http://hts.sp.nitech.ac.jp/archives/2.3/HTS-2.3_for_HTK-3.4.1.tar.bz2 --no-check-certificate
tar xvjf HTS-2.3_for_HTK-3.4.1.tar.bz2

cd $DOWNLOAD_PATH
wget http://htk.eng.cam.ac.uk/ftp/software/HTK-3.4.1.tar.gz --user=$HTKUSER --password=$HTKPASSWORD  --no-check-certificate
wget http://htk.eng.cam.ac.uk/ftp/software/hdecode/HDecode-3.4.1.tar.gz --user=$HTKUSER --password=$HTKPASSWORD  --no-check-certificate
...
wget https://nchc.dl.sourceforge.net/project/hts-engine/hts_engine%20API/hts_engine_API-1.10/hts_engine_API-1.10.tar.gz  --no-check-certificate
